#!/usr/bin/env python3
"""
Command-line interface for BERiT (Biblical Hebrew RoBERTa) embedding search.
"""
...
# berit_cli.py -> dense/ -> understanding_embeddings/ -> project root
project_root = Path(__file__).parent.parent.parent
if str(project_root) not in sys.path:
    sys.path.insert(0, str(project_root))

from understanding_embeddings.dense.vector_store import create_vector_store, load_vector_store
from understanding_embeddings.dense.search import dense_search
from understanding_embeddings.shared.verse_parser import parse_verse_reference, get_hebrew_for_verses
from understanding_embeddings.dense.models import get_persist_directory, get_outputs_directory
...
OUTPUTS_DIR = get_outputs_directory(Path(__file__).parent / 'outputs', MODEL_KEY)
...
def load_vector_store_safe(record_level: str = 'verse'):
    ...
    if not persist_dir.exists():
        ...
        print(f"Run 'python berit_cli.py index' first to create the index")
        return None
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Search Hebrew text using BERiT (Biblical Hebrew RoBERTa) embeddings'
    )
    ...
    if args.command == 'index':
        base_dir = Path(__file__).parent
        # BERiT only supports verse level
        persist_dir = get_persist_directory(base_dir, MODEL_KEY, 'verse')
        ...
        print("Creating BERiT vector store index (verse level)...")
        create_vector_store(DATA_DIR, model_key=MODEL_KEY, record_level='verse', force=args.force)
        print("✓ Indexing complete!")
        
    elif args.command == 'search':
        # BERiT uses verse level
